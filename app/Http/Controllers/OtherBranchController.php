...
class OtherBranchController extends Controller
{
    ...
    {
        $excludedBranches = ['cebu', 'makati', 'bohol', 'pampanga'];

        $managerProfiles = ManagerProfile::whereNotIn('branch', $excludedBranches)->get();
        $employeeProfiles = EmployeeProfile::whereIn('id', $managerProfiles->pluck('id'))->get();
        $acceptedRecruits = ContactUs::where('status', 'accepted')
            ->whereNotIn('branch', $excludedBranches)
            ->get();

        return view('members.other', [
