...
@section('content')
    ...
    <div class="max-w-full mx-auto bg-white p-6 rounded-lg shadow-md"
        ...
        >
                @foreach ($managerProfiles->whereNotIn('branch', ['cebu', 'makati', 'bohol', 'negros-oriental', 'pampanga']) as $manager)
                <div class="flex-shrink-0" style="width: 350px;">
                    <div class="bg-white p-4 border-black-1 rounded-lg shadow-md flex flex-col justify-between text-center"
                        ...
                        <div>
                            <h1 class="text-lg font-semibold mt-2">{{ $manager->specify_branch }} Branch</h1>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add click event listener to open employee modal button
    ...
    <script src="{{ asset('js/branch-js/other-branch.js') }}"></script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        if (event.target.closest('.editEmployeeBtn')) {
            ...
            editForm.action = `/members/updateEmployeeProfile/${employeeId}`;
    ...
    if (editEmployeeModalElement) {
        editEmployeeModalElement.addEventListener('hidden.bs.modal', function () {
            document.getElementById('editEmployeeForm').reset();
            document.getElementById('edit_educationback').innerHTML = '';
            document.getElementById('edit_keyskills').innerHTML = '';
        });
    }
});
</script>
...
<script>
// Replace the showEmployees function with this updated version
    ...
    fetch(`/managers/${managerId}/employees`)
        ...
        .catch(error => {
            ...
            if (employeeContainer) {
                employeeContainer.innerHTML = '<p class="text-center text-red-500 w-full">Error loading employees. Please try again later.</p>';
                employeeContainer.classList.remove('hidden');
            }
        });
}
